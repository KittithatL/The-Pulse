...
export default function Login() {
  ...
  const [form, setForm] = useState({
    emailOrName: "", // ปรับให้ตรงกับ Backend ที่รอรับ emailOrName หรือ emailOrUsername
    password: "",
  });

  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(""); // ✅ เพิ่ม State สำหรับเก็บ Error Message
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result?.success) {
        // ✅ ใช้ window.location เพื่อรีเฟรชสถานะ Auth ใหม่ทั้งหมด ป้องกันหน้าขาว
        window.location.href = "/projects"; 
      } else {
        // ✅ แสดงข้อความเมื่อ Login ไม่สำเร็จ (เช่น Invalid credentials)
        ...
        <div className="flex items-center justify-center px-6 py-10 sm:px-10 lg:px-14">
          <div className="w-full max-w-md">
            ...
            <div className="mb-10 flex items-center gap-4">
              <img src={logoPImg} alt="Logo" className="w-[105px] h-[105px] object-contain" />
              <div className="pt-1">
                <div className="text-4xl font-extrabold italic text-black">The Pulse</div>
            ...
            <form onSubmit={handleSubmit} className="mt-8 space-y-6">
              ...
              <div>
                <div className="flex items-center justify-between">
                  ...
                  <button type="button" className="text-xs font-extrabold text-red-600 hover:opacity-80">FORGOT PASSWORD?</button>
                </div>
                <div className="mt-2 flex items-center gap-3 rounded-xl border border-slate-200 bg-white px-4 py-4 shadow-sm">
                  <div className="grid h-11 w-11 place-items-center rounded-lg bg-slate-100">
                    <svg viewBox="0 0 24 24" className="h-6 w-6 text-slate-400" fill="none" stroke="currentColor" strokeWidth="2">
              ...
              <div className="pt-2 text-center text-sm font-semibold text-slate-400">
                No system access?{" "}
                <button type="button" onClick={() => navigate("/register")} className="font-extrabold text-red-600 hover:opacity-80">Sign up</button>
              </div>
            </form>
          </div>
        </div>
